...
Relay.prototype.find = function(startAddr, endAddr) {
  ...
  while (addr <= end) {
    ...
    commond += this.validate.crc16('8F100101' + addrS + '00008E') + ','
    addr++
  }
  ...
  return {
    ...
    resolve: function(result, success, error) {
      ...
      // if(result.substr(0,2)!="7f"||result.substr(-2,2)!="7e"){
      //     return error(401);
      // }
...
Relay.prototype.write = function(addr, code, state) {
  ...
  var commond = '6F100101' + addr + code + state + '6E'
  ...
  return {
    ...
    resolve: function(result, success, error) {
      ...
      if (result.substr(2, 6) !== '100101') {
        return error(402)
      }
      if (result.substr(0, 2) !== '7f' || result.substr(-2, 2) !== '7e') {
        return error(401)
      }
      if (result.substr(8, 2) !== addr) {
        return error(403)
      }
      var data = result.substr(12, 2)
      if (data === '64')data = 1
