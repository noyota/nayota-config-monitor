 ...
 Relay.prototype.find = function(startAddr, endAddr) {
  ...
  while (addr <= end) {
    ...
    while (addrS.length < 2) {
      ...
      var jiaoyanone=(239+254+parseInt(addrS,16)+1).toString(16);
      var jiaoyan=jiaoyanone.substr(jiaoyanone.length-2,jiaoyanone.length);
    commond += this.validate.crc16("EFFE"+addrS+"01"+jiaoyan+"0D0A") + ','
    addr++
  }
...
Relay.prototype.read = function(addr, code) {
  ...
  code.forEach(function(item) {
    ...
    var jiaoyanone=(239+254+parseInt(addr,16)+1).toString(16);
    var jiaoyan=jiaoyanone.substr(jiaoyanone.length-2,jiaoyanone.length);
    var commond="EFFE"+addr+"01"+jiaoyan+"0D0A";
  var cmd = [commond]
    cmd.push()
  var validate = this.validate
  return {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (cmd[i] === commond) {
          ...
          if (func !== '06') {
            ...
            allChecks['1'] = item.substr(20,8);
            allChecks['2'] = item.substr(36,8);
            allChecks['3'] = item.substr(52,8);
            allChecks['4'] = item.substr(68,8);
            allChecks['5'] = item.substr(84,8);
            allChecks['6'] = item.substr(100,8)
