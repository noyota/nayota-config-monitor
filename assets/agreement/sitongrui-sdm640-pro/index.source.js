...
Relay.prototype.read = function(addr, code) {
  ...
  var commond1 = this.validate.crc16(addr + '03002A0001')
  var commond2 = this.validate.crc16(addr + '03002B0001')
  var commond3 = this.validate.crc16(addr + '03000A0002')
  var cmd = []
  ...
  return {
    ...
    resolve: function(result, success, error) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (addrback !== addr.toLowerCase()) { // 验证型号是否正确
          return error(403)
        }
        var validatedata = validate.crc16(item)
        if (validatedata.toLowerCase() !== data[i].toLowerCase()) { // crc16验证传输过程是否正确没有丢包
          return error(401)
        }
        if (cmd[i] === commond1) {
          ...
          allChecks['2a'] = item.substr(6, 4)
        } else if (cmd[i] === commond2) {
          const func = item.substr(2, 2)
          if (func !== '03') {
            return error(402)
          }
          allChecks['2b'] = item.substr(6, 4)
...
Relay.prototype.encode = function(addr, parameters, port) {
  /**
          ...
          *cmd3:查询电能
          */
  var cmd1 = this.validate.crc16(addr + '03002A0001')
  var cmd2 = this.validate.crc16(addr + '03002B0001')
  var cmd3 = this.validate.crc16(addr + '03000A0002')
  /**
