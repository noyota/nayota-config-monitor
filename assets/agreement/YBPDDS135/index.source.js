...
Relay.prototype.read = function(addr, code, attribute) {
  ...
  var commond1 = this.validate.crc16(addr + '0300000002') // 电能
  var cmd = []
  cmd.push(commond1)
  var validate = this.validate
  var pars = {}
  ...
  return {
    ...
    resolve: function(result, success, error) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        // var func2=data[i].(0, data[i].length - 2)
        ...
        // var db = parseInt(data[i].successubstr(18, 2), 16)
        // res['0']= db
      }
      code.forEach(function(item, index) {
        ...
        if (allChecks[item] && analyze) {
          if (item === '1') {
            res[item] = parseFloat(xs * analyze(allChecks[item]))
          } else {
