...
Relay.prototype.find = function(startAddr, endAddr) {
  ...
  while (addr <= end) {
    ...
    commond += this.validate.crc16(addrS + '0300330001') + ','
    addr++
    // console.log("----"+commond)
  }
  ...
  var attribute = []
  // console.log("123="+defaultOperates)
...
Relay.prototype.changeAddr = function(options) {
  ...
  // var jiaoyan = jiaoyanone.substr(jiaoyanone.length - 2, 2)
  var commond = options.oldAddr+ '06002000' + options.shortAddress
...
Relay.prototype.read = function(addr, code) {
  ...
  var commond1=this.validate.crc16(addr+'030030001C')
    ...
    resolve: function(result, success, error) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        // console.log("item="+item)
          ...
          allChecks['1']=item.substr(10, 2)       //模式
          allChecks['2']=item.substr(18, 2)       //风速
          allChecks['3']=item.substr(46, 2)       //开关状态
          allChecks['4']=item.substr(22, 2)       //室内温度
          allChecks['5']=item.substr(34, 2)       //面板温度设定值
          allChecks['6']=item.substr(54, 2)       //室内湿度
          allChecks['7']=item.substr(58, 2)       //室外温度
          allChecks['8']=item.substr(114, 2)      //新风状态
          allChecks['9']=item.substr(86, 2)      //地暖温度
      }
...
Relay.prototype.write = function(addr, code, state) { // 01 05 00 03 FF 00 7C 3A 线圈4 01 05 00 03 00 00 3D CA
  ...
  if (typeof state === 'number')state = state.toString(16)
  ...
  if(code=='01'){            //空调模式
    state='31'
  }else if(code=='02'){      //空调风速
    state='32'
  }else if(code=='03'){      //开关状态
    state='39'
  }else if(code=='05'){      //空调面板温度
    state='36' 
  }else if(code=='08'){      //排风状态
    state='4B'
  }else if(code=='09'){      //地暖温度
    state='43'
  }
  // if(state==0){
  //   state='03'
  ...
  // var jiaoyan = jiaoyanone.substr(jiaoyanone.length - 2, 2)
  var commond =addr + '0600'+state +'00'+ state2
  commond = this.validate.crc16(commond)
  var validate = this.validate
  // console.log(commond)
  var cmd = [commond]
  return {
    ...
    resolve: function(result, success, error) {
      ...
      //   rdata = state2-50
      // }
      rdata=parseInt(rdata,16);
      // if(code=='01'){
      //   rdata=rdata.toString(2); 
      //   while(rdata.length<8){
      //     rdata='0'+rdata;
      //   }
      //   rdata=rdata.substr(2,2); 
      //   rdata=parseInt(rdata,2);
      // }
