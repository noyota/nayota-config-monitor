...
Relay.prototype.find = function(startAddr, endAddr) {
  ...
  return {
    ...
    resolve: function(result, success, error) {
      ...
      var func=result.substr(16,2);
      if(func!='93'){
          return error(402);
      }
      var address = result.substr(32, 2)
      var json = {
...
Relay.prototype.changeAddr = function(options) {
  ...
  var commond = '68AAAAAAAAAAAA6815063333333333' + addre + jiaoyan + '16'
  var devicename = this.options.name
  var defaultCheck = this.options.defaultCheck
  var attribute = []
  if (this.options.attribute !== undefined) {
...
Relay.prototype.read = function(addr, code) {
  ...
  function getCommond(codeone) {
    ...
    var jiaoyan = jiaoyanone.substr(jiaoyanone.length - 2, 2)
    var commond1 = '680000000000' + addr + '681104' + codeone + jiaoyan + '16'
    return commond1
  }
  // console.log(cmd)
  return {
    ...
    resolve: function(result, success, error) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        var func = item.substr(16, 2)
        if (func !== '91') {
          return error(402)
        }
        var cheak=null
        if (item.substr(20,8)=='33343435'){
            cheak=1
        }else if (item.substr(20,8)=='33333333'){
            cheak=2
        }else if (item.substr(20,8)=='33343535'){
            cheak=3
        }
        if (code.indexOf('1') > -1 && cheak==1) { // 电压
          if (result.length < 36) {
            return error(400)
          }
          let data1 = item.substr(28, 2)
          let data2 = item.substr(30, 2)
        ...
        } else if (code.indexOf('2') > -1 && cheak==2) { // 电能
          if (result.length < 40) {
            return error(400)
          }
          let data1 = item.substr(28, 2)
          let data2 = item.substr(30, 2)
          let data3 = item.substr(32, 2)
          let data4 = item.substr(34, 2)
          data1 = (parseInt(data1, 16) - 51).toString(16)
          data2 = (parseInt(data2, 16) - 51).toString(16)
          ...
          allChecks['2'] = data4 + data3 + data2 + data1
            // console.log("checks2="+allChecks['2'])
        } else if (code.indexOf('3') > -1 && cheak==3) { // 电流
          if (result.length < 38) {
            return error(400)
          }
          let data1 = item.substr(28, 2)
