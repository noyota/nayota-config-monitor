...
Relay.prototype.read = function(addr, code) {
  ...
  if (code.indexOf('0') > -1) {
    codeone = '0000'
    var commond1 = this.validate.crc16(addr + '03' + codeone + '0002')
    cmd.push(commond1)
  }
  if (code.indexOf('2') > -1) {
    codeone = '0002'
    var commond2 = this.validate.crc16(addr + '03' + codeone + '0002')
    cmd.push(commond2)
  }
  if (code.indexOf('3') > -1) {
    codeone = '0003'
    var commond3 = this.validate.crc16(addr + '03' + codeone + '0002')
    cmd.push(commond3)
  }
  return {
    cmd: cmd.join(','),
    resolve: function(result, success, error) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if (cmd[i] === commond3) {
          allChecks['3'] = item.substr(6, 8)
        }
        var db = parseInt(data[i].substr(18, 2), 16)
        res['0'] = db
      }
