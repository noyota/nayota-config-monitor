...
Relay.prototype.find = function(startAddr, endAddr) {
  ...
  return {
    ...
    resolve: function(result, success, error) {
      ...
      if (result.substr(0, 4) !== '8983') {
        return error(401)
      }
      const valid = validate.he(result.substr(0, result.length - 2))
...
Relay.prototype.read = function(addr, code) {
  ...
  var commond1 = this.validate.crc16(addr + '0300070002')
  var cmd = []
  if (code.indexOf('7') > -1) {
    cmd.push(commond1)
  }
...
Relay.prototype.decode = function(result) {
  var ret = {}
  if (result.length === 16) {
    const data = result.substr(0, 14)
    const valid = this.validate.he(data).toLowerCase()
