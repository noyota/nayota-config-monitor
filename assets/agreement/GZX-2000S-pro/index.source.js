...
Relay.prototype.find = function(startAddr, endAddr) {
  ...
  while (addr <= end) {
    ...
    commond += this.validate.crc16(addrS + '0305A10001') + ','
    addr++
  }
  ...
  return {
    ...
    resolve: function(result, success, error) {
      ...
      validatedata = validate.crc16(fun);
...
Relay.prototype.read = function(addr, code) {
  ...
  if (code.indexOf('0') > -1) {
    addr1 = '0000'
	var commond1 = this.validate.crc16(addr + '03' + addr1 + '0002')
    cmd.push(commond1)
  }
  if (code.indexOf('136') > -1) {
    addr1 = '007c'
	var commond2 = this.validate.crc16(addr + '03' + addr1 + '0002')
    cmd.push(commond2)
  }
  if (code.indexOf('140') > -1) {
    addr1 = '007e'
	var commond3 = this.validate.crc16(addr + '03' + addr1 + '0002')
    cmd.push(commond3)
  }
  if (code.indexOf('144') > -1) {
    addr1 = '0090'
	var commond4 = this.validate.crc16(addr + '03' + addr1 + '0002')
    cmd.push(commond4)
  }
  return {
    cmd: cmd.join(','),
    resolve: function(result, success, error) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        validatedata = validate.crc16(func1);
		...
		if(cmd[i] === commond2){
			allChecks['136']=item.substr(6,8)				
		}
		if(cmd[i] === commond3){
			allChecks['140']=item.substr(6,8)				
		}
		if(cmd[i] === commond4){
			allChecks['144']=item.substr(6,8)				
		}
