<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="name" :label="$t('table.name')" sortable="custom" align="center" width="180" />
      <el-table-column :label="$t('HardWareWord.type')" align="center" prop="type" width="100px">
        <template slot-scope="scope">
          <span>{{ scope.row.type===0 ? $t('HardWareWord.equ'):scope.row.type===1?'LORA':scope.row.type===2?'HTTP':scope.row.type===4?'LORAWAN':scope.row.type===5?'云网关设备':'TCP/UDP设备' }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('HardWareWord.code')" align="center" sortable="custom" prop="code" width="180px" />
      <el-table-column :label="$t('HardWareWord.agreement')" align="center" prop="agreement">
        <template slot-scope="scope">
          <span>{{ scope.row.agreement ? scope.row.agreement.name:'' }}</span>
        </template>
      </el-table-column>
...
import { list, deleteOne, deleteMany } from '@/api/hardwareword'
// import { list as controlList } from '@/api/controls'
// import { oldData } from '@/api/hardwareword'
...
export default {
  ...
  data() {
    return {
      ...
      fieldOptions: [
        ...
        { key: 'code', label: this.$t('HardWareWord.type') }
      ],
      listQuery: {
        label: 'name',
        search: '',
  ...
  methods: {
    ...
    // copyData() {
    //   oldData(this.temp).then((response) => {
    //   })
    // }
